...
private object NoopFrameController : FrameController<Any> {
  private var topFrame by mutableStateOf<FrameController.BackstackFrame<Any>?>(null)

  override val activeFrames: List<FrameController.BackstackFrame<Any>>
    get() = topFrame?.let { listOf(it) } ?: emptyList()

  override fun updateBackstack(keys: List<Any>) {
    topFrame = FrameController.BackstackFrame(keys.last())
  }
}